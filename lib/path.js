...
function path(p) {
	...
	this.isIn = function(p) {
		if (!(p instanceof path)) return this.isIn(path(p));
	...
	this.join = function(p) {
		if (!(p instanceof path)) return this.join(path(p));
	...
	this.relative = function(p) {
		if (!(p instanceof path)) return this.relative(path(p));
		if (p.isRelative()) {
			if (this.isFile()) {
	...
	this.path = function() {
		...
		return path(this.parts.slice(0, this.parts.length - 1).join(sep)).join('/');
	};
	...
	this.newer = function(other, cb) {
		if (!(other instanceof path)) return this.newer(path(other), cb);
